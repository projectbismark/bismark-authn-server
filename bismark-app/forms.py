...
class InfoForm(forms.Form):
    ...
    state = forms.CharField(label="State / province", max_length=75, widget=forms.TextInput(attrs=dict(maxlength=75)))
    country = forms.ChoiceField(label="Country", choices=COUNTRIES)
    unknown_plan = forms.BooleanField(required=False)
    node_id = forms.CharField(label="Router MAC address", required=False)
